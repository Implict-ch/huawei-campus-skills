...
**「会考 XX 吗 / 考不考 XX」**（AI 岗，如「华为会考线代吗」）— **强制**：

1. **先 Read** `choice-topic-aliases.yaml` 映射口语 → 大类 tag1，再 Read `stats.json` 的 `choice_bank_tag1_counts`、`choice_bank_tag2_counts`、`choice_bank_representatives`（**禁止**说「统计文件查不到」；数据在 `stats.json`）
2. **结论**：会考 + 选择题专项题库该大类题数/占比 + 常见小类（1 句）
3. **具体怎么做第一步必须是「代表真题（选择题）」表**，**3–4 行**，题号列为可点击超链接（与编程题同规格），**至少 2 道**
4. **禁止**：只给大类入口链接、只列小类名称不给单题链接、**单独「刷题链接」列或「去刷题」文案**、**用编程题代表表替代选择题代表表**
5. 编程侧若 `index.json`/`tag-index` 也有命中，可在选择题表**之后**加「编程题侧（补充）」**最多 1 道**，不得顶掉选择题代表表

| 文件 | 用途 |
|------|------|
| `choice-no-leak.md` | **选择题题面防泄露（Agent 必读）**；频次/推题前先读 |
| `choice-topic-aliases.yaml` | 口语别名 → 选择题大类 tag1；**「会考线代吗」类必先 Read** |
...
**选择题统计类回答**（如「线代选择题考几次」「大模型选择题怎么练」「AI 选择题七大类」「华为会考线代吗」）：
...
| 问题类型 | 同条回答必须包含 |
|----------|------------------|
| **单一大类**（如「线代选择题考几次」「华为会考线代吗」） | ① **结论**（大类题数、占全库占比）→ ② **小类分布表**（该大类下**每个小类**：题数 + **占该类比例**，数据来自 `choice_bank_tag2_stats`）→ ③ **代表真题表**（3–4 行）→ ④ 考法归纳 1–2 句 → ⑤ 备考建议 1–2 句 |
| **单一小类**（如「特征值分解考几道」） | 结论含小类题数 + **类内占比** + 所属大类 → 代表题表 3–4 行 → 考法/备考各 1–2 句 |
| **概览类**（「AI 选择题考哪些」） | 七大类频次表 → **每个大类一节小类分布表**（各小类题数 + 类内占比）→ Top 3 大类各 1 道代表真题 |
...
- 大类刷题入口：[线性代数专项](https://codefun2000.com/choice/hw#tag1=线性代数)
...
**选择题单一考点回答骨架（示例：华为会考线代吗 / 线代选择题考几次）**：
...
```markdown
❌ 只给「线性代数专项」大类链接，代表真题表却是编程题
❌ 代表真题表用「刷题链接」列 + 「去刷题」——应与编程题一样题号列即超链接
❌ 说「统计文件里查不到小题数」——应 Read stats.json choice_bank_tag1_counts
✅ 结论含 131 道 + 代表题（选择题）表 3–4 行具体链接 + 考法/备考
```
